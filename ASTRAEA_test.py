...
def main():
    ...
    #print("Target Proposition Submitted: ", target_prop_id)

    ''' voting phase '''
    print("-- Voting Phase --\n")
    # the submitter will vote as well (as honest voter)
    tx_hash = contract.functions.voting_request(voter_stake_max).transact({'from': submitter})
    ...
    contract.functions.vote(prop_id, hashed_vote, vote_id).transact({'from': submitter})
    ...
    for i in range(voters):
        voter = web3.eth.accounts[i]
        contract.functions.reveal_voter_sealed_vote(voters_prop_voted[i], vote_id, voters_salt[i]).transact({'from': voter})
